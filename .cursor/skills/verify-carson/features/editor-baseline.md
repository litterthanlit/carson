...
- **Skip intro.** Choose `New poster`, then `Skip intro`. The driver does this before every feature. The dialog closes and region `Poster canvas` is visible.
...
- Dialog `Wreck this poster` can lazy-load. Wait for the Posters dashboard, choose `New poster`, wait for the dialog, then `Skip intro`, then wait until it is hidden. Do not click inspector tabs while `.command-backdrop` is up.
