...
- `home-empty` shows `No saved posters yet` and `Start a poster` when this browser has no saves.
- `home-start` opens the seeded editor from that empty control.
...
- Empty studio: choose `Start a poster`, Save, then `Home`.
...
- **Land on Home.** Choose `Skip intro`. Region `Home` is visible. Region `Poster canvas` is absent. Copy includes `No saved posters yet`. Button `Start a poster` is present.
- **Start from empty.** Choose `Start a poster`. Region `Poster canvas` is visible. Heading `Carson` is present. Layers include `Oversized headline`.
...
- Editor recipes auto-click `Start a poster` after Skip intro. This feature must not.
