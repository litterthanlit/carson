...
const FEATURES = {
  ...
  async 'file-identity'(page, outDir) {
    ...
    await home.getByRole('button', { name: 'Open Night bus' }).click()
    await page.getByRole('region', { name: 'Poster canvas' }).waitFor({ state: 'visible' })
    ...
    await page.getByRole('button', { name: 'Home', exact: true }).click()
    await home.waitFor({ state: 'visible' })
    await home.getByRole('button', { name: 'Open Night bus' }).waitFor()
    await home.getByRole('button', { name: 'Open Night bus copy' }).waitFor()
    await capture(page, outDir, 'duplicated-grid')

    await home.getByRole('button', { name: 'Open Day plaza' }).click()
    await page.getByRole('region', { name: 'Poster canvas' }).waitFor({ state: 'visible' })
    ...
    await home.waitFor({ state: 'visible' })
    await home.getByRole('button', { name: 'Open Night bus' }).waitFor()
    await home.getByRole('button', { name: 'Open Night bus evening' }).waitFor()
    await capture(page, outDir, 'saved-as-grid')

    await home.getByRole('button', { name: 'Open Night bus evening' }).click()
    await page.getByRole('region', { name: 'Poster canvas' }).waitFor({ state: 'visible' })
    await page.getByRole('button', { name: 'Save as' }).click()
    await saveAs.waitFor()
    ...
    await page.getByRole('button', { name: 'Home', exact: true }).click()
    await home.waitFor({ state: 'visible' })
    await home.getByRole('button', { name: 'Recover Untitled poster' }).waitFor()
